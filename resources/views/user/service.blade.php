...
{{--meta tags--}}
@section('meta')
    <title>CODEI SYSTEMS | {{ $service->name }}</title>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1" />
    <meta name="author" content="{{ $service->meta_author }}">
    <!-- description -->
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-9 col-md-7 col-sm-11 col-xs-12 text-center center-col">
                    <h4 class="alt-font text-white center-col margin-lr-auto xs-width-100 xs-margin-15px-bottom">{{ $service->name }}</h4>
                    <p class="margin-lr-auto sm-width-100 xs-margin-25px-bottom">{{ $service->short_description }}</p>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center center-col height-650px overflow-hidden md-height-auto md-bottom--minus50 md-position-relative wow fadeIn xs-bottom-0" style="visibility: visible; animation-name: fadeIn;">
                    <img src="{{ Storage::url($service->media) }}" alt="{{ $service->title }}" data-no-retina="">
                </div>
            </div>
        </div>
    </section>
